...
console.log(
	`  Squared proportion sum: ${result.concentration.squaredProportionsSum.toFixed(3)}`,
);
console.log(`  Active markets: ${result.concentration.activeMarketCount}`);
console.log(
	`  Effective market count: ${result.concentration.effectiveMarketCount.toFixed(1)}`,
);
...
console.log(
	`  Weighted avg: ${(result.utilizationExposure.weightedAvg * 100).toFixed(1)}%`,
);
console.log(
	`  Max single market: ${(result.utilizationExposure.max * 100).toFixed(1)}%`,
);
...
console.log(
	`  Single oracle dominant: ${result.oracle.isSingleOracleDominant}`,
);

// Collateral diversity: how many different collateral tokens back the vault?
console.log("\n=== Collateral Diversity ===");
console.log(
	`  Distinct collaterals: ${result.collateralDiversity.distinctCount}`,
);
console.log(
	`  Squared proportion sum: ${result.collateralDiversity.squaredProportionsSum.toFixed(3)}`,
);

// Governance: timelock, guardian, curator, and fees
console.log("\n=== Governance ===");
console.log(
	`  Timelock: ${result.governance.timelockSeconds}s (${result.governance.timelockTier})`,
);
...
console.log("\n=== Cap Headroom ===");
console.log(
	`  Weighted avg utilization: ${(result.capHeadroom.weightedAvgUtilization * 100).toFixed(1)}%`,
);
console.log(
	`  Tightest market: ${result.capHeadroom.tightest?.marketId ?? "none (all unlimited)"}`,
);
